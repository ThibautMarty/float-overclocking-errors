...
Clkwiz::Clkwiz(std::vector<float> _frequencies)
  : frequencies(_frequencies)
{
  vectormode = true;

  // They need to be in order for settings selection
  std::sort(frequencies.begin(), frequencies.end());

  // findSettings needs those values
  maxfreq = frequencies.back() + 2.f; // Cross the bound to avoid missing the value in findSettings
  minfreq = frequencies[0] - 2.f; // Idem
  // For compatibility when ENABLE_CLKWIZ is unset
  step = (maxfreq - minfreq) / 10.f;
...
void Clkwiz::findSettings()
{
  ...
  {
    std::cerr << "clkwiz debug: 0 = " << frequencies[0] << '\n';
    std::cerr << "clkwiz debug: 1 = " << frequencies[1] << '\n';
    // Select one solution by target frequency
    settings.reserve(frequencies.size());
    std::vector<float>::iterator current = frequencies.begin();
    for(it = tmp.begin(); (it != tmp.end()) && (current != frequencies.end()); it++)
    {
      if((*it)->out >= *current)
      {
        std::cerr << "clkwiz debug: select conf (" << (*it)->out << ") for goal (" << (*current) << ")\n";
        settings.push_back(*it);
        current++;
      }
      else
